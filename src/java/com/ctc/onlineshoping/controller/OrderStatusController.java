 ...
 * @author Gulshan
 */
@WebServlet(name = "OrderStatusController", urlPatterns = {"/OrderStatusController"})
public class OrderStatusController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String orderid = request.getParameter("readyid");
        ChangeOrderStatusDAO change = new ChangeOrderStatusDAO();
        boolean status = change.changeStatus(orderid);
        
        if(status){
            
            ManageAccountDAO manage = new ManageAccountDAO();
            manage.addToAccount(orderid);
            response.sendRedirect("merchantorderstatus.jsp");
